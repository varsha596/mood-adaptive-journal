# Mood Adaptive Journal

Live Demo  : 

Description  :
 A frontend web application to track daily moods and the reasons behind them. Users can select their mood (happy, sad, angry, calm), write a short note, and visualize mood trends over time.

Features  :  
 Add, edit, and delete daily mood entries  
 Track mood trends with color-coded visual representation  
 Filter entries by mood or search by text  
 Responsive and interactive UI  
 LocalStorage support for saving entries locally  

Tech Stack  :  
 HTML, CSS, JavaScript  
 LocalStorage for data persistence  

How to Use Locally  :
1. Clone the repository  

